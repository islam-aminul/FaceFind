...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existingResult = await client.models.FaceRecognitionConfig.list({
      filter: { configKey: { eq: DEFAULT_CONFIG_KEY } },
    });
    ...
    const configData = { configKey: DEFAULT_CONFIG_KEY, ...body };
    let result;

    if (existingResult.data && existingResult.data.length > 0) {
      result = await client.models.FaceRecognitionConfig.update({
        id: existingResult.data[0].id,
        ...configData,
      });
    } else {
      result = await client.models.FaceRecognitionConfig.create(configData);
    }
